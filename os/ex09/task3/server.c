...
void *listener_thread_func(void *arg_pointer)
{
    ...
    char ** usernames = calloc(MAX_USER, sizeof(char *));
    ...
    while (1)
    {
        ...
        client_args->usernames = usernames;
...
void *client_thread_func(void *arg_pointer)
{
    ...
    args->usernames[args->index] = username;
    size_t length;
    ...
    while (1)
    {
        ...
        if (!shutting_down)
        {
            ...
            {
                length = 0;
                while(buffer[length + 3] != ' ' && buffer[length + 3] != '\0'){
                    length++;
                }
               // Send message to all connected users
                ...
                {   
                    ...
                    if (args->usernames[i] != NULL && strlen(args->usernames[i]) == length && !(args->connections[i] == myconnection))
                    {
                        if(strncmp(args->usernames[i], (buffer + 3), length) == 0){
                            dprintf(args->connections[i], "%s (whispers): %s", username, buffer + 4 + length);
                            break;
                        }
                    }
                } 
