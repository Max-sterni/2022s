...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < csv.length; i++) {
            ...
            String[] attributes = csv_row.split(",", 5);
            String[] name = attributes[0].split(" ");

            String[] adressStrings = attributes[1].split(";");
            String[] streetAndNumber = adressStrings[0].split(" ");
            Address address = new Address(streetAndNumber[0], streetAndNumber[1], Integer.parseInt(adressStrings[1]), adressStrings[2], adressStrings[3]);

            String[] hobbiessWithName = attributes[3].split(":");
            String[] hobbies = hobbiessWithName[1].split(";");

            for (int j = 0; j < hobbies.length; j++) {
                hobbies[j] = hobbies[j].toLowerCase();
            }

            persons.add(new Person(name[0], name[1], address, attributes[2], hobbies));
